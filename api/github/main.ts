...
function createSession(token: string): string {
  const payload = base64url(JSON.stringify({ token, expiresAt: Date.now() + 60 * 60 * 8 * 1000 }));
...
export default async function handler(req: any, res: any) {
  ...
  // FIXED: Use clean path without query parameters
  ...
  if (req.url?.includes('/callback') || action === 'callback') {
    ...
    try {
      ...
      return redirect(res, '/?github=connected', [clearState, cookie(SESSION_COOKIE, createSession(token), { maxAge: 60 * 60 * 8 })]);
    } catch (error: any) {
  ...
  if (action === 'repos') {
    try {
      ...
      return res.status(200).json({ repos: repos.map((r: any) => ({ id: r.id, full_name: r.full_name, private: r.private, default_branch: r.default_branch, html_url: r.html_url })) });
    } catch {
  ...
  if (action === 'contents') {
    ...
    if (!owner || !repo || !/^[A-Za-z0-9_.-]+$/.test(String(owner)) || !/^[A-Za-z0-9_.-]+$/.test(String(repo))) {
      return res.status(400).json({ error: 'A valid owner and repository are required.' });
    }
    const ref = req.query.ref ? `?ref=${encodeURIComponent(String(req.query.ref))}` : '';
    try {
      const result = await requestGitHub(
        `/repos/${encodeURIComponent(String(owner))}/${encodeURIComponent(String(repo))}/contents/${filePath.split('/').map(encodeURIComponent).join('/')}${ref}`,
        session.token
      );

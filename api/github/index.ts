...
function createSession(token: string): string {
  const payload = base64url(JSON.stringify({ token: token, expiresAt: Date.now() + SESSION_TTL_SECONDS * 1000 }));
...
function readSession(req: any): { token: string; expiresAt: number } | null {
  ...
  if (!safeEqual(parts[1], sign(parts[0], getSessionSecret()))) return null;
  try {
...
function requestGitHub(path: string, token: string | null, method: string, body?: any): Promise<any> {
...
function requireSession(req: any, res: any): any {
...
async function buildStatus(req: any): Promise<any> {
  ...
  const results = await Promise.all([
    requestGitHub('/user', session.token, 'GET'),
    requestGitHub('/user/repos?sort=updated&per_page=5', session.token, 'GET'),
  ]);
  ...
  return {
    ...
    user: {
      ...
      name: results[0].name || results[0].login,
    },
    repos: results[1].map((r: any) => ({
      id: r.id,
...
export default async function handler(req: any, res: any) {
  ...
  if (action === 'callback') {
    ...
    try {
      ...
      await requestGitHub('/user', token, 'GET');
  ...
  try {
    if (action === 'repos') {
      const repos = await requestGitHub('/user/repos?sort=updated&per_page=30', session.token, 'GET');
      return json(res, 200, {
    ...
    if (action === 'contents') {
      ...
      if (!owner || !repoName || !/^[A-Za-z0-9_.-]+$/.test(owner) || !/^[A-Za-z0-9_.-]+$/.test(repoName)) {
      ...
      const path = '/repos/' + encodeURIComponent(owner) + '/' + encodeURIComponent(repoName) + '/contents/' + filePath.split('/').map(encodeURIComponent).join('/') + ref;
      return json(res, 200, await requestGitHub(path, session.token, 'GET'));
    }
    if (action === 'search') {
      const query = String(req.query.q || '').trim();
      if (!query || query.length > 200) return json(res, 400, { error: 'A short repository search query is required.' });
      return json(res, 200, await requestGitHub('/search/repositories?q=' + encodeURIComponent(query) + '&per_page=5', session.token, 'GET'));
    }

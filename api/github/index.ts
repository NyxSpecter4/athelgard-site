  ...
  return new Promise((resolve, reject) => {
    ...
    const request = https.request({
      ...
      response.on('end', () => {
        ...
        } catch { reject(new Error('Could not read GitHub authorization response')); }
      });
    });
    request.on('error', reject);
...
export default async function handler(req: any, res: any) {
  ...
  try {
    if (action === 'status') {
      ...
      return json(res, 200, { connected: true, user: { login: user.login, avatar_url: user.avatar_url, name: user.name || user.login } });
    }
    if (action === 'repos') {

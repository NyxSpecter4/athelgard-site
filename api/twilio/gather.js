// /api/twilio/gather — proxy to bountywarz.com ka-voice reply loop
...
module.exports = async function handler(req, res) {
  ...
  const url = UPSTREAM + '/api/ka-voice' + (req.url || '').replace('/api/twilio/gather', '') + '&step=reply&mode=default';
  ...
  try {
    ...
    console.error('[gather] upstream fail:', e.message);
    res.statusCode = 200;
